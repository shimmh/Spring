...
//    @PostMapping("/add")
    public String addItem1(@ModelAttribute("item") Item item, Model model) {
        itemRepository.save(item);
       // model.addAttribute("item", item);// ModelAttribute("item")를 사용하면 model에 item이라는 키값으로 데이터가 add됨
        return "/basic/item";
    }

    //@PostMapping("/add")
    public String addItem2(@ModelAttribute Item item, Model model) {
        //Item -> item 이라는 키로 model에 담김
        itemRepository.save(item);
        return "/basic/item";
    }
